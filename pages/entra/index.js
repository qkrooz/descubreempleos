...
const Login = React.memo(() => {
  ...
  // context
  const { ResetInfo, SetInfo } = useContext(MainContext);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      if (localStorage.getItem("userInfo") === null) {
        ...
        if (Object.values(JSON.parse(localStorage.getItem("userInfo"))).length !== 0 && Object.values(JSON.parse(localStorage.getItem("secondaryInfo"))).length !== 0) {
          Router.push("/");
        } else {
          ResetInfo();
          Router.push("/entra");
    ...
    <>
      ...
      <div className={style.container}>
        <div className={style.logoContainer}>
          <img src="https://descubrempleos.com/webServices/img/logo1.png" alt="logo1" />
        </div>
        <Formik
            ...
            <Form className={style.formContainer}>
              <div className={style.fields}>
                <div>
                  ...
                  {errors.EMAIL ? <span className={style.error}>Email requerido *</span> : null}
                </div>
                <div>
                  <div className={style.pswField}>
                    <Field type={pswtext} name="PASSWORD" onChange={handleChange} value={values.PASSWORD} />
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  {errors.PASSWORD ? <span className={style.error}>Contraseña requerida *</span> : null}
                </div>
              </div>
              <button type="submit" children="INICIAR" className={style.blackButton} />
        ...
        <div className={style.captionContainer}>
          <span>{"\u00a9"}Todos los derechos reservados - Descubre Sa. de CV</span>
        </div>
      </div>
    </>
  );
